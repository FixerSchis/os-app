...
def initialize_database():
    ...
    try:
        ...
        if any(keyword in error_str for keyword in ['table', 'already exists', 'duplicate', 'exists']):
            ...
            if 'alembic_version' not in tables:
                logger.info("Database exists but alembic_version table not found. "
                           "This appears to be an existing database without migration tracking.")
                
                # Check if we have any application tables (indicating this is a real database)
                app_tables = [t for t in tables if t not in ['sqlite_sequence']]  # Exclude SQLite system tables
                
                if app_tables:
                    logger.info(f"Found existing tables: {app_tables}")
                    logger.info("Stamping database as current head to enable future migrations...")
                    ...
                    logger.info("No application tables found. This appears to be a fresh database.")
                    raise e
            else:
