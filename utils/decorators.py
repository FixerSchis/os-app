...
def roles_required(roles=None, condition_func=None):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # If roles were specified but the user doesn't have them, and no custom condition was met
            if roles and not custom_condition_met:
                flash(f"You require one of the following roles to access this page: {', '.join(roles)}.")
                abort(403)
...
def character_owner_or_user_admin_required(f):
    """
    Decorator that checks if the current user is a 'user_admin' or the owner of the character.
    ...
    def decorated_function(*args, **kwargs):
        ...
        if character.player_id == current_user.player_id or current_user.has_role(Role.USER_ADMIN.value):
            return f(*args, **kwargs)
...
def character_owner_or_downtime_team_required(f):
    """
    Decorator that checks if the current user is a 'downtime_team' or the owner of the character.
    It expects 'character_id' in the view arguments.
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
