...
class PrintLayout:
    ...
    def calculate_layout(self, item_width: float, item_height: float, template: PrintTemplate, is_back: bool = False, generate_cut_guides: bool = True) -> Dict:
        """
        ...
        margin_right = template.margin_left_mm if is_back else template.margin_right_mm
        ...
        if generate_cut_guides:
            cut_guides = self._generate_cut_guides(
                margin_left, template.margin_top_mm,
                item_width, item_height,
                template.items_per_row, template.items_per_column,
                template.gap_horizontal_mm, template.gap_vertical_mm
            )
    ...
    def _generate_cut_guides(
        ...
        items_per_row: int, items_per_column: int,
        gap_horizontal: float, gap_vertical: float
    ) -> List[Dict]:
        ...
        for col in range(items_per_row + 1):
            ...
            guides.append({
                ...
                'height': items_per_column * item_height + (items_per_column - 1) * gap_vertical + 10
            })
    ...
    def generate_pdf(self, items: List[Dict], template: PrintTemplate, double_sided: bool = False) -> BytesIO:
        """
        ...
        back_layout = self.calculate_layout(width, height, template, is_back=True) if double_sided else None
    ...
    def generate_character_sheets_pdf(self, characters: List) -> BytesIO:
        ...
        for character in characters:
            ...
            items_to_print.append({
                ...
                'back_html': template.get_back_page_render(template_context) if template.has_back_side else None,
    ...
    def generate_exotic_substance_labels_pdf(self, items: List) -> BytesIO:
        """Generate a PDF containing a full sheet of identical exotic substance labels."""
