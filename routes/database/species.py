...
def species_list():
    ...
    if not current_user.is_authenticated or not current_user.has_role(Role.RULES_TEAM.value):
        ...
        # Filter species to only show those with player character factions or user's species
        filtered_species = []
        for s in species:
...
def edit_species(species_id):
    ...
    return render_template('species/edit.html', species=species, BodyHitsType=BodyHitsType, factions=factions, AbilityType=AbilityType, skills_list=skills_list)
...
def edit_species_post(species_id):
    ...
    if not all([name, wiki_page, body_hits_type, body_hits, death_count, permitted_factions]):
        ...
        return render_template('species/edit.html', species=species, BodyHitsType=BodyHitsType, factions=Faction.query.all(), AbilityType=AbilityType, skills_list=Skill.query.all())
