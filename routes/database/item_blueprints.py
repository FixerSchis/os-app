...
def list():
    ...
    return render_template('rules/item_blueprints/list.html', blueprints=blueprints, can_edit=can_edit, mod_instances_by_blueprint=mod_instances_by_blueprint)
...
def create_post():
    ...
    try:
        ...
        for mod_id, count in mod_counts.items():
            db.session.execute(text('INSERT INTO item_blueprint_mods (item_blueprint_id, mod_id, count) VALUES (:bid, :mid, :count)'), {
                'bid': blueprint.id,
                'mid': mod_id,
                'count': count
            })
...
def edit(id):
    ...
    return render_template('rules/item_blueprints/edit.html', blueprint=blueprint, item_types=item_types, mods=mods_dict, initial_mods=initial_mods)
...
def edit_post(id):
    ...
    if not name or not item_type_id or not blueprint_id or not base_cost:
        flash('All fields are required', 'error')
        return render_template('rules/item_blueprints/edit.html', blueprint=blueprint, item_types=item_types, mods=mods_dict)
    try:
        blueprint.name = name
        ...
        db.session.execute(
            item_blueprint_mods.delete().where(item_blueprint_mods.c.item_blueprint_id == blueprint.id)
        )
        # Group mod IDs and insert with count
        mod_counts = Counter([int(mid) for mid in mods_applied_ids])
        for mod_id, count in mod_counts.items():
