...
def list():
    ...
    if mod_id:
        ...
        query = query.outerjoin(item_blueprint_mods, item_blueprint_mods.c.item_blueprint_id == BP.id)
        query = query.filter(
            or_(
    ...
    if expiry_filter == 'expired':
        ...
        query = query.filter(Item.expiry == None)
    ...
    if sort_by == 'item_id':
        ...
        query = query.order_by(
            BP.name if sort_order == 'asc' else BP.name.desc()
        )
    elif sort_by == 'expiry':
        query = query.order_by(
            Item.expiry if sort_order == 'asc' else Item.expiry.desc()
        )
    ...
    return render_template('rules/items/list.html',
        items=items,
...
def create_post():
    ...
    try:
        ...
        max_item = Item.query.filter_by(blueprint_id=blueprint_id).order_by(Item.item_id.desc()).first()
        ...
        for mod_id, count in mod_counts.items():
            db.session.execute(text('INSERT INTO item_mods_applied (item_id, mod_id, count) VALUES (:iid, :mid, :count)'), {
                'iid': item.id,
                'mid': mod_id,
                'count': count
            })
...
def edit(id):
    ...
    for bp in blueprints.values():
        ...
        mod_counts = [(Mod.query.get(row.mod_id), row.count) for row in mod_rows]
        mod_instances_by_blueprint[bp.id] = mod_counts
    return render_template('rules/items/edit.html', item=item, blueprints=blueprints, mods=mods_dict, initial_mods=initial_mods, mod_instances_by_blueprint=mod_instances_by_blueprint)
...
def edit_post(id):
    ...
    try:
        ...
        db.session.flush()
        db.session.execute(
            item_mods_applied.delete().where(item_mods_applied.c.item_id == item.id)
        )
        mod_counts = Counter([int(mid) for mid in mods_applied_ids])
        for mod_id, count in mod_counts.items():
            db.session.execute(text('INSERT INTO item_mods_applied (item_id, mod_id, count) VALUES (:iid, :mid, :count)'), {
...
def find_by_code():
    ...
    requires_pack = request.values.get('requires_pack', 'false').lower() in ['1', 'true', 'yes']
    if not full_code:
    ...
    blueprint = ItemBlueprint.query.filter_by(item_type_id=item_type.id, blueprint_id=blueprint_id).first()
    if not blueprint:
        return jsonify({'error': 'Item not found'}), 404
    # Find the item
    ...
    if requires_pack:
        packs_with_item = DowntimePack.query.filter(DowntimePack.items.contains([str(item.id)])).all()
