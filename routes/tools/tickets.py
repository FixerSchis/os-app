...
def list_tickets():
    ...
    upcoming_character_tickets = EventTicket.query.join(Event).filter(
        EventTicket.character_id.in_(character_ids),
        Event.end_date >= now
    ).order_by(Event.start_date.asc()).all()

    # Past tickets for characters
    past_character_tickets = EventTicket.query.join(Event).filter(
        EventTicket.character_id.in_(character_ids),
        Event.end_date < now
    ...
    upcoming_assigned_tickets = EventTicket.query.join(Event).filter(
        EventTicket.assigned_by_id == current_user.id,
        Event.end_date >= now
    ).order_by(Event.start_date.asc()).all()
    
    # Past tickets assigned by user
    past_assigned_tickets = EventTicket.query.join(Event).filter(
        EventTicket.assigned_by_id == current_user.id,
        Event.end_date < now
    ).order_by(Event.start_date.desc()).all()
