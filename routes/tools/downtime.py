...
def get_available_science_slots_with_sources(character, science_type=None):
    ...
    current_pack = next((pack for pack in character.downtime_packs if pack.status == DowntimeTaskStatus.ENTER_DOWNTIME), None)
...
def index():
    ...
    if not active_period:
        return render_template('downtime/index.html', 
                             active_period=None,
    ...
    if current_user.has_role('downtime_team'):
        ...
        default_status = next((status for status in DowntimeTaskStatus if status.value in packs_by_status), None)
    ...
    # For regular users
        ...
        user_packs = [pack for pack in active_period.packs 
                     if pack.character.user_id == current_user.id 
                     and pack.status == DowntimeTaskStatus.ENTER_DOWNTIME]
        
        # If user has exactly one character in enter_downtime state, redirect to enter downtime
        if len(user_packs) == 1:
            return redirect(url_for('downtime.enter_downtime', 
...
def enter_pack_contents(period_id, character_id):
    ...
    return render_template('downtime/enter_pack_contents.html',
        period_id=period_id,
        ...
        if pack.conditions:
            for condition in pack.conditions:
                if condition and condition not in pack.character.active_conditions:
                    pack.character.active_conditions.append(CharacterCondition(
        ...
        if pack.cybernetics:
            for cybernetic_id in pack.cybernetics:
                ...
                if cybernetic and cybernetic not in pack.character.cybernetics:
                    db.session.add(CharacterCybernetic(
                        character_id=pack.character.id,
                        cybernetic_id=cybernetic_id
                    ))
...
def enter_downtime(period_id, character_id):
    ...
    if character.group:
        group_packs = DowntimePack.query.filter(
            ...
            DowntimePack.character_id.in_([c.id for c in character.group.members if c.id != character.id])
        ).all()
    ...
    blueprints_data = [{'id': bp.id, 'name': bp.name, 'base_cost': bp.base_cost} for bp in blueprints]

    if character.group:
        available_samples = character.group.samples.order_by(Sample.name).all()
    else:
        available_samples = Sample.query.filter(Sample.id.in_(pack.samples)).order_by(Sample.name).all() if pack.samples else []
    ...
    if character.group:
        ...
        group_projects = CharacterResearch.query.filter(CharacterResearch.character_id.in_(group_member_ids)).all()
        group_members = [c for c in character.group.members if c.id != character.id]
    
    # Convert CharacterResearch objects to dictionaries
    my_projects = [{'research_id': cr.research.public_id, 'project_name': cr.research.project_name, 'character_name': cr.character.name if cr.character else None} for cr in my_projects]
    group_projects = [{'research_id': cr.research.public_id, 'project_name': cr.research.project_name, 'character_name': cr.character.name if cr.character else None} for cr in group_projects]
    pack_items = [{'id': item.id, 'name': item.blueprint.name, 'type': item.blueprint.item_type.id, 'full_code': item.full_code} for item in pack_items]
    group_items = [{'id': item.id, 'name': item.blueprint.name, 'type': item.blueprint.item_type.id, 'full_code': item.full_code} for item in group_items]
    available_samples = [{'id': sample.id, 'name': sample.name, 'type': sample.type.value, 'tags': [tag.name for tag in sample.tags], 'is_researched': sample.is_researched} for sample in available_samples]
    pack_exotics = [{'id': exotic['id'], 'name': ExoticSubstance.query.get(exotic['id']).name, 'amount': exotic['amount']} for exotic in pack.exotic_substances]
    ...
    return render_template('downtime/enter_downtime.html',
        character=character,
        pack=pack,
...
def manual_review(period_id, character_id):
    ...
    exotics = [
        {"id": substance.id, "name": substance.name}
        for substance in ExoticSubstance.query.all()
    ]
    ...
    if not pack.review_data.get("stages_json") or pack.review_data.get("stages_json") == '[]':
        pack.review_data["stages_json"] = json.dumps([{"stage_number": 1, "name": "Stage 1", "description": "", "unlock_requirements": []}])
        ...
        for engineering in pack.engineering:
            if engineering.action == 'modify':
                ...
                if (
                    ...
                    pack.character.character_pack['downtime_results'][str(pack.id)].append(
                        f"Could not apply {mod.name} to {item.name} - character does not know the modification"
                    )
                    ...
                    if target_research and not target_research.is_complete():
                        ...
                        if current_stage:
                            ...
                            for req_progress in current_stage.requirement_progress:
                                if (req_progress.requirement.requirement_type == ResearchRequirementType.SCIENCE and 
                                    req_progress.requirement.science_type == science_type and
                                    req_progress.progress < req_progress.requirement.amount):
                                    req_progress.progress += 1
                                    pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                        f"Made progress on {target_research.research.project_name}"
                                    )
                                    made_progress = True
                                    break
                            if not made_progress:
                                pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                    f"Failed to make progress on {target_research.research.project_name} for {target_research.character.name}"
                                )
                        else:
                            pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                f"No current stage for {target_research.research.project_name} for {target_research.character.name}"
                            )
                    elif target_research.is_complete():
                        pack.character.character_pack['downtime_results'][str(pack.id)].append(
                            f"Research already complete for {target_research.research.project_name} for {target_research.character.name}"
                        )
                    else:
                        pack.character.character_pack['downtime_results'][str(pack.id)].append(
                    ...
                    if teaching_research and target_research and not target_research.is_complete():
                        ...
                        if target_current_stage:
                            ...
                            if teaching_stage:
                                ...
                                pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                    f"Successfully taught {target_research.research.project_name} to {target_research.character.name}"
                                )
                            else:
                                pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                    f"Failed to teach {target_research.research.project_name} to {target_research.character.name} - no stages to teach"
                                )
                    else:
                        pack.character.character_pack['downtime_results'][str(pack.id)].append(
                            f"Failed to teach {target_research.research.project_name} for {target_research.character.name} - target has completed this project"
                        )
                        ...
                        if research.get('contributed_exotics'):
                            for exotic in research.get('contributed_exotics'):
                                ...
                                if exotic_id and quantity > 0:
                                    ...
                                    for req_progress in current_stage.requirement_progress:
                                        if (req_progress.requirement.requirement_type == ResearchRequirementType.EXOTIC and 
                                            req_progress.requirement.exotic_substance_id == substance.id and
                                            req_progress.progress < req_progress.requirement.amount):
                                            # Add progress
                                            ...
                                            for pack_exotic in pack.exotic_substances:
                                                if pack_exotic.get('id') == exotic_id:
                                                    ...
                                                    break
                                            pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                                f"Contributed {quantity} {substance.name} to {target_research.research.project_name} for {research_character.name}"
                                            )
                                            break
                        ...
                        if research.get('contributed_items'):
                            for item_id in research.get('contributed_items'):
                                if item_id:
                                    ...
                                    # Find matching requirement
                                    for req_progress in current_stage.requirement_progress:
                                        if (req_progress.requirement.requirement_type == ResearchRequirementType.ITEM and 
                                            req_progress.requirement.item_type == item.blueprint.item_type.id and
                                            req_progress.progress < req_progress.requirement.amount):
                                            # Add progress
                                            req_progress.progress += 1
                                            # Remove from pack
                                            for pack_item in pack.items:
                                                if pack_item.get('id') == item_id:
                                                    ...
                                                    break
                                            pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                                f"Contributed {item.blueprint.name} to {target_research.research.project_name} for {research_character.name}"
                                            )
                                            break
                        ...
                        if research.get('contributed_samples'):
                            for sample_id in research.get('contributed_samples'):
                                if sample_id:
                                    ...
                                    # Find matching requirement
                                    for req_progress in current_stage.requirement_progress:
                                        if (req_progress.requirement.requirement_type == ResearchRequirementType.SAMPLE and 
                                            req_progress.requirement.sample_tag == sample.tag and
                                            req_progress.progress < req_progress.requirement.amount):
                                            # Add progress
                                            req_progress.progress += 1
                                            # Remove from pack
                                            for pack_sample in pack.samples:
                                                if pack_sample.get('id') == sample_id:
                                                    ...
                                                    break
                                            pack.character.character_pack['downtime_results'][str(pack.id)].append(
                                                f"Contributed {sample.name} to {target_research.research.project_name} for {research_character.name}"
                                            )
                                            break
                        ...
                        if existing_research:
                            ...
                            if char_research:
                                ...
                                if all_complete:
                                    ...
                                    stages_data = json.loads(manual.get('stages_json', '[]'))
                                    if stages_data:
                                        new_stage_data = stages_data[0]  # Only use first stage for improvement
        ...
        for research in character_research:
            ...
            if current_stage:
                ...
                if current_stage.meets_requirements():
                    ...
                    pack.character.character_pack['downtime_results'][str(pack.id)].append(
                        f"Completed stage '{current_stage.name}' of {research.research.project_name}"
                    )
                ...
                if current_stage.stage_completed:
                    ...
                    if next_stage:
                        pack.character.character_pack['downtime_results'][str(pack.id)].append(
                            f"Advanced to stage '{next_stage.name}' in {research.research.project_name}"
                        )
                    else:
                        pack.character.character_pack['downtime_results'][str(pack.id)].append(
