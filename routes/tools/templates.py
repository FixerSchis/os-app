...
def print_item_sheet(type, id):
    """Generate PDF for a single item (character sheet, item card, condition card, etc.)"""
...
def print_event_items(event_id, type):
    ...
    try:
        if type == 'characters':
            ...
            characters = Character.query.join(EventTicket).filter(EventTicket.event_id == event_id).all()
            if not characters:
...
def generate_template_completions(template_type):
    ...
    if template_type == PrintTemplateType.CHARACTER_SHEET:
        ...
        for column in ItemType.__table__.columns:
            if column.name not in ['id']:
                item_type_fields[f'blueprint.item_type.{column.name}'] = f"{column.type} - {column.name}"
...
def print_template_preview(template_id):
    ...
    try:
        ...
        if template.type == PrintTemplateType.CHARACTER_SHEET:
            ...
            pdf = layout_manager.generate_character_sheets_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        elif template.type == PrintTemplateType.CHARACTER_ID:
            sample = get_sample_character()
            pdf = layout_manager.generate_character_id_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        elif template.type == PrintTemplateType.ITEM_CARD:
            sample = get_sample_item()
            pdf = layout_manager.generate_item_cards_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        elif template.type == PrintTemplateType.MEDICAMENT_CARD:
            sample = get_sample_medicament()
            pdf = layout_manager.generate_medicament_sheet_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        elif template.type == PrintTemplateType.CONDITION_CARD:
            sample = get_sample_condition()
            pdf = layout_manager.generate_condition_sheet_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        elif template.type == PrintTemplateType.EXOTIC_SUBSTANCE_LABEL:
            sample = get_sample_exotic_substance()
            pdf = layout_manager.generate_exotic_substance_labels_pdf([sample for _ in range(template.items_per_row * template.items_per_column)])
        else:
...
def template_layout(template_id):
    ...
    return render_template("templates/layout.html", template=template, PrintTemplateType=PrintTemplateType)