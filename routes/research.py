...
def research_create():
    ...
    return render_template('research/edit.html', research=None, items=items, blueprints=blueprints, item_types=item_types, exotics=exotics, ResearchType=ResearchType, ScienceType=ScienceType, ResearchRequirementType=ResearchRequirementType)
...
def research_create_post():
    ...
    if type_value == 'artefact':
        if artefact_option == 'existing':
            ...
            if not item_type_id:
                flash('You must select an item type to create a new artefact item.', 'error')
                items = Item.query.join(ItemBlueprint).order_by(ItemBlueprint.name).all()
                blueprints = ItemBlueprint.query.order_by(ItemBlueprint.name).all()
                item_types = ItemType.query.order_by(ItemType.name).all()
                return render_template('research/edit.html', research=None, items=items, blueprints=blueprints, item_types=item_types, ResearchType=ResearchType, ScienceType=ScienceType, ResearchRequirementType=ResearchRequirementType)
            # Create new blueprint
            max_blueprint = ItemBlueprint.query.filter_by(item_type_id=item_type_id).order_by(ItemBlueprint.blueprint_id.desc()).first()
            ...
            max_item = Item.query.filter_by(blueprint_id=new_blueprint.id).order_by(Item.item_id.desc()).first()
...
def research_edit(research_id):
    ...
    return render_template('research/edit.html', research=research, items=items, blueprints=blueprints, item_types=item_types, exotics=exotics, sample_tags=sample_tags, ResearchType=ResearchType, ScienceType=ScienceType, ResearchRequirementType=ResearchRequirementType)
...
def research_edit_post(research_id):
    ...
    if research.type.value == 'artefact':
        if artefact_option == 'existing':
            ...
            if not item_type_id:
                ...
                blueprints = ItemBlueprint.query.order_by(ItemBlueprint.name).all()
                item_types = ItemType.query.order_by(ItemType.name).all()
                return render_template('research/edit.html', research=research, items=items, blueprints=blueprints, item_types=item_types, ResearchType=ResearchType, ScienceType=ScienceType, ResearchRequirementType=ResearchRequirementType)
            max_blueprint = ItemBlueprint.query.filter_by(item_type_id=item_type_id).order_by(ItemBlueprint.blueprint_id.desc()).first()
            next_blueprint_id = (max_blueprint.blueprint_id + 1) if max_blueprint else 1
            new_blueprint = ItemBlueprint(
    ...
    existing_stages = list(ResearchStage.query.filter_by(research_id=research.id).order_by(ResearchStage.stage_number).all())
    ...
    for idx, stage_data in enumerate(stages_data, 1):
        if idx <= existing_stage_count:
            ...
            existing_reqs = { (r.requirement_type.value, getattr(r, 'science_type', None) and r.science_type.value, r.item_type, r.sample_tag, r.requires_researched, r.amount): r for r in stage.unlock_requirements }
            new_reqs_keys = set()
            ...
            for key, req in existing_reqs.items():
                if key not in new_reqs_keys:
                    # Remove any CharacterResearchStageRequirement referencing this requirement
...
def api_blueprints():
    ...
    results = [
        {'id': bp.id, 'text': f'{bp.name} ({bp.item_type.id_prefix}{bp.blueprint_id:04d})'}
...
def assignees(research_id):
    ...
    available_characters = Character.query.filter(~Character.id.in_(assigned_character_ids), Character.status == 'active').all()
    
    return render_template('research/assignees.html', 
                         research=research,
                         assignees=assignees,
...
def edit_progress(research_id, character_id):
    ...
    return render_template('research/edit_progress.html',
...
def edit_progress_post(research_id, character_id):
    ...
    if request.form.get('advance_stage'):
        ...
        return redirect(url_for('research.edit_progress', research_id=research_id, character_id=character_id))
...
def project_info():
    ...
    if target_id:
        ...
        if '.' in target_id:
            ...
            cr = CharacterResearch.query.filter_by(research_id=project.id, character_id=char_id).first()
            if cr and cr.character.player_id == player_id:
                result['valid'] = True
                result['character_name'] = cr.character.name
                current_stage = cr.get_current_stage()
                if current_stage:
                    result['stage_requirements'] = [req_to_dict(req) for req in current_stage.stage.unlock_requirements]
            else:
                # If the character is the current user or a downtime_team user, allow access
                char = Character.query.get(char_id)
                if char and char.player_id == player_id and (char.user_id == current_user.id or current_user.has_role(Role.DOWNTIME_TEAM.value)):
                    result['valid'] = True
                    result['character_name'] = char.name
                    # If not assigned to project yet, use first stage requirements
                    if not cr:
                        first_stage = ResearchStage.query.filter_by(research_id=project.id).order_by(ResearchStage.stage_number).first()
                        if first_stage:
                            result['stage_requirements'] = [req_to_dict(req) for req in first_stage.unlock_requirements]
                    else:
                    ...
                    result['error'] = 'Character not assigned to this project or player ID mismatch.'
        else:
            result['error'] = 'Invalid character ID format.'
    return jsonify(result)
...
def can_teach_character():
    ...
    teaching_research = CharacterResearch.query.filter_by(
        character_id=teaching_character_id,
    ...
    if '.' in character_id:
        try:
            ...
            return jsonify({'found': False, 'valid': False, 'error': 'Invalid character ID format.'})
...
def view_research_list():
    """Display a view-only list of research projects for the current user's characters."""
    # Get all research projects assigned to the user's characters
    user_research = CharacterResearch.query.join(Character).filter(
    ...
    return render_template('research/view_list.html', 
                         research_by_character=research_by_character)
...
def view_research_project(research_id):
    ...
    character_research = CharacterResearch.query.join(Character).filter(
    ...
    return render_template('research/view_project.html',
                         research=research,
                         character_research=character_research,
                         current_stage=current_stage,
                         stages=stages,
