...
def generate_public_id():
    ...
    return '-'.join(random.sample(wordlist, 3))
...
class Research(db.Model):
    ...
    stages = db.relationship('ResearchStage', backref='research', cascade='all, delete-orphan', order_by='ResearchStage.stage_number')
    ...
    def assign_character(self, character_id):
        ...
        # Get the first stage
        first_stage = ResearchStage.query.filter_by(
            research_id=self.id
        ).order_by(ResearchStage.stage_number).first()
        ...
        if first_stage:
            ...
            for req in first_stage.unlock_requirements:
                req_progress = CharacterResearchStageRequirement(
                    character_research_stage_id=stage_progress.id,
    ...
    def stages_to_json(self):
        stages = ResearchStage.query.filter_by(research_id=self.id).order_by(ResearchStage.stage_number).all()
        ...
        for assignee in assignees:
            if assignee.current_stage_id is not None:
                ...
                # If current_stage_id is None, the assignee is complete (beyond all stages)
        ...
        for stage in stages:
            ...
            result.append({
                ...
                'unlock_requirements': [{
                    ...
                    'amount': req.amount,
                    'sample_tag': req.sample_tag if req.requirement_type.value == 'sample' else None,
                    'requires_researched': req.requires_researched if req.requirement_type.value == 'sample' else False
...
class ResearchStage(db.Model):
    ...
    unlock_requirements = db.relationship('ResearchStageRequirement', backref='stage', cascade='all, delete-orphan')
...
class ResearchStageRequirement(db.Model):
    ...
    requirement_type = db.Column(db.Enum(
        ResearchRequirementType,
        native_enum=False,
        values_callable=lambda obj: [e.value for e in obj]
    ), nullable=False)
    science_type = db.Column(db.Enum(
        ScienceType,
        native_enum=False,
        values_callable=lambda obj: [e.value for e in obj]
    ), nullable=True)
    item_type = db.Column(db.String(100), nullable=True)
    exotic_substance_id = db.Column(db.Integer, db.ForeignKey('exotic_substances.id'), nullable=True)
    ...
    def __repr__(self):
        return f'<ResearchStageRequirement {self.requirement_type} {self.amount}>'
...
class CharacterResearch(db.Model):
    ...
    character = db.relationship('Character', backref=db.backref('character_research', lazy='dynamic'))
    research = db.relationship('Research', backref=db.backref('character_research', lazy='dynamic'))
    progress = db.relationship('CharacterResearchStage', backref='character_research', cascade='all, delete-orphan')
    ...
    def get_current_stage(self):
        """Get the current stage of the research, or None if current_stage_id is null."""
        if not self.current_stage_id:
            return None
        return CharacterResearchStage.query.filter_by(
            character_research_id=self.id,
    ...
    def advance_stage(self):
        ...
        # Find the next stage
        next_stage = ResearchStage.query.filter(
        ...
        if next_stage:
            ...
            for req in next_stage.unlock_requirements:
                req_progress = CharacterResearchStageRequirement(
                    character_research_stage_id=new_stage.id,
                    requirement_id=req.id
                )
                db.session.add(req_progress)
    ...
    def regress_stage(self):
        ...
        prev_stage = ResearchStage.query.filter(
            ResearchStage.research_id == self.research_id,
            ResearchStage.stage_number < current_stage.stage.stage_number
        ).order_by(ResearchStage.stage_number.desc()).first()
...
class CharacterResearchStage(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    character_research_id = db.Column(db.Integer, db.ForeignKey('character_research.id'), nullable=False)
    stage_id = db.Column(db.Integer, db.ForeignKey('research_stage.id'), nullable=False)
    ...
    requirement_progress = db.relationship('CharacterResearchStageRequirement', backref='stage', cascade='all, delete-orphan')
    ...
    def __repr__(self):
        return f'<CharacterResearchStage {self.character_research_id} - {self.stage_id}>'
...
class CharacterResearchStageRequirement(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    character_research_stage_id = db.Column(db.Integer, db.ForeignKey('character_research_stage.id'), nullable=False)
    requirement_id = db.Column(db.Integer, db.ForeignKey('research_stage_requirement.id'), nullable=False)
    progress = db.Column(db.Integer, nullable=False, default=0)
    
    requirement = db.relationship('ResearchStageRequirement', backref='character_progress', lazy='joined')

    def __repr__(self):
        return f'<CharacterResearchStageRequirement {self.character_research_stage_id} - {self.requirement_id}>' 