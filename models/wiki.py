...
class WikiPageVersion(db.Model):
    ...
    page_slug = db.Column(
        db.String(200),
        db.ForeignKey("wiki_page.slug", ondelete="CASCADE"),
        nullable=False,
    )
    version_number = db.Column(db.Integer, nullable=False)
    status = db.Column(
        SqlEnum(WikiPageVersionStatus),
        nullable=False,
        default=WikiPageVersionStatus.PUBLISHED,
    )
...
class WikiSection(db.Model):
    ...
    version_id = db.Column(
        db.Integer,
        db.ForeignKey("wiki_page_version.id", ondelete="CASCADE"),
        primary_key=True,
    )
...
wiki_changelog_versions = db.Table(
    ...
    db.Column(
        "changelog_id",
        db.Integer,
        db.ForeignKey("wiki_change_log.id"),
        primary_key=True,
    ),
    db.Column(
        "version_id",
        db.Integer,
        db.ForeignKey("wiki_page_version.id"),
        primary_key=True,
    ),
)
...
class WikiChangeLog(db.Model):
    ...
    versions = db.relationship(
        "WikiPageVersion", secondary=wiki_changelog_versions, backref="change_logs"
    )
...
wiki_page_tags = db.Table(
    ...
    db.Column(
        "tag_id",
        db.Integer,
        db.ForeignKey("wiki_tag.id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
def get_or_create_wiki_page(slug, default_title, created_by=None):
    ...
    section = WikiSection(version_id=version.id, id=1, order=0, title=default_title, content="")
