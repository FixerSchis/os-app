...
class Skill(db.Model):
    ...
    science_type = db.Column(db.Enum(ScienceType, values_callable=lambda x: [e.value for e in x], native_enum=False), nullable=True)  # Only used if adds_science_downtime > 0
    ...
    required_factions = db.Column(db.String, nullable=True)  # Stored as JSON string of faction values
    ...
    def character_meets_requirements(self, character):
        ...
        if self.required_skill_id:
            ...
            has_prerequisite = any(
                skill.skill_id == self.required_skill_id 
