...
def test_render_email_template(mock_env, mock_loader, app):
    ...
    with app.app_context():
        ...
        mock_env_instance.get_template.side_effect = [
            mock_html_template,
            mock_text_template,
        ]
        ...
        text_body, html_body = render_email_template(
            "test_template", user={"name": "Test User"}, url="https://example.com"
        )
        ...
        mock_html_template.render.assert_called_once_with(
            user={"name": "Test User"}, url="https://example.com"
        )
...
def test_send_notification_email_success(mock_render, mock_send, app):
    ...
    with app.app_context():
        ...
        # Verify template was rendered
        mock_render.assert_called_once_with(
            "notification_downtime_completed", user=mock_user, downtime={"id": 1}
        )
