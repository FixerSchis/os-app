...
def test_index_redirect(app, test_client):
...
def test_500_error_handler(app, test_client):
    """Test the 500 error handler."""
    original_testing_config = app.config['TESTING']
    app.config['TESTING'] = False

    with app.test_request_context(), mock.patch('routes.wiki.render_template', side_effect=Exception("mocked error")):
...
def test_has_enter_downtime_packs_context_processor_no_packs(app, test_client, authenticated_user):
...
def test_has_enter_downtime_packs_context_processor_with_pack(app, test_client, authenticated_user, downtime_pack):
...
def test_has_research_projects_context_processor_no_projects(app, test_client, authenticated_user):
...
def test_has_research_projects_context_processor_with_project(app, test_client, authenticated_user, character_research):
...
def test_chrome_devtools_json(app, test_client):
