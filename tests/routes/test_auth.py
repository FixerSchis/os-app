...
from models.enums import Role, CharacterStatus, TicketType
...
from models.wiki import WikiPage, WikiPageVersion, WikiPageVersionStatus
...
def test_register_success_first_user(test_client, db, wiki_index_page):
    ...
    cybernetic = Cybernetic(name="Neural Interface", neural_shock_value=0, wiki_slug="neural-interface")
...
def test_verification_required_authenticated_verified(test_client, authenticated_user, db, wiki_index_page):
...
def test_reset_password_get_valid_token(test_client, db):
    ...
    user = User(
        email='reset_token_test@example.com',
        first_name='Test',
        surname='User'
    )
    user.set_password('password123')
...
def test_reset_password_success(test_client, db):
    ...
    with patch.object(user, 'reset_password', return_value=True) as mock_reset:
        with patch('routes.auth.datetime') as mock_datetime:
            mock_datetime.now.return_value = user.reset_token_expires - timedelta(hours=1)
            response = test_client.post(f'/auth/reset-password/{user.reset_token}', data={
...
def test_reset_password_missing_fields(test_client, db):
    ...
    with patch('routes.auth.datetime') as mock_datetime:
        mock_datetime.now.return_value = user.reset_token_expires - timedelta(hours=1)
        response = test_client.post(f'/auth/reset-password/{user.reset_token}', data={}, follow_redirects=True)
        assert response.status_code == 200
