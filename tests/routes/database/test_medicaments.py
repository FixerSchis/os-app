...
def test_medicaments_create_post_authorized(test_client, verified_login, db):
    ...
    response = test_client.post('/db/medicaments/new', data=post_data, follow_redirects=True, content_type='application/x-www-form-urlencoded')
    assert response.status_code == 200
...
def test_medicaments_create_missing_fields(test_client, verified_login, db):
    ...
    response = test_client.post('/db/medicaments/new', data=post_data, content_type='application/x-www-form-urlencoded')
    assert response.status_code == 200
...
def test_medicaments_edit_post_authorized(test_client, verified_login, medicament, db):
    ...
    response = test_client.post(f'/db/medicaments/{medicament.id}/edit', data=post_data, follow_redirects=True, content_type='application/x-www-form-urlencoded')
    assert response.status_code == 200
...
def test_medicaments_edit_missing_fields(test_client, verified_login, medicament, db):
    ...
    response = test_client.post(f'/db/medicaments/{medicament.id}/edit', data=post_data, content_type='application/x-www-form-urlencoded')
    assert response.status_code == 200
    assert b'Name and wiki page are required' in response.data
