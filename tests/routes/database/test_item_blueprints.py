...
def test_item_blueprints_list_authorized(test_client, rules_team_user, db):
    with test_client as c:
        ...
        response = c.get('/db/item-blueprints/')
        assert response.status_code == 200
        assert b'Blueprint' in response.data or b'blueprint' in response.data
...
def test_item_blueprints_create_get_authorized(test_client, rules_team_user, db):
    with test_client as c:
        ...
        response = c.get('/db/item-blueprints/create')
        assert response.status_code == 200
        assert b'Blueprint' in response.data or b'blueprint' in response.data

def test_item_blueprints_create_post_authorized(test_client, rules_team_user, item_type, mod_obj, db):
    with test_client as c:
        c.post('/auth/login', data={
            'email': rules_team_user.email,
            'password': 'password123',
        ...
        response = c.post('/db/item-blueprints/create', data=post_data, follow_redirects=True, content_type='application/x-www-form-urlencoded')
        assert response.status_code == 200
...
def test_item_blueprints_create_missing_fields(test_client, rules_team_user, item_type, db):
    with test_client as c:
        ...
        response = c.post('/db/item-blueprints/create', data=post_data, content_type='application/x-www-form-urlencoded')
        assert response.status_code == 200
...
def test_item_blueprints_edit_get_authorized(test_client, rules_team_user, item_blueprint_obj, db):
    with test_client as c:
        ...
        response = c.get(f'/db/item-blueprints/{item_blueprint_obj.id}/edit')
        assert response.status_code == 200
        assert bytes(item_blueprint_obj.name, 'utf-8') in response.data

def test_item_blueprints_edit_post_authorized(test_client, rules_team_user, item_blueprint_obj, item_type, mod_obj, db):
    with test_client as c:
        c.post('/auth/login', data={
            'email': rules_team_user.email,
            'password': 'password123',
        ...
        response = c.post(f'/db/item-blueprints/{item_blueprint_obj.id}/edit', data=post_data, follow_redirects=True, content_type='application/x-www-form-urlencoded')
...
def test_item_blueprints_edit_missing_fields(test_client, rules_team_user, item_blueprint_obj, item_type, db):
    with test_client as c:
        c.post('/auth/login', data={
            'email': rules_team_user.email,
            'password': 'password123',
        ...
        response = c.post(f'/db/item-blueprints/{item_blueprint_obj.id}/edit', data=post_data, content_type='application/x-www-form-urlencoded')
        assert response.status_code == 200
        assert b'All fields are required' in response.data
...
def test_item_blueprints_edit_not_found(test_client, rules_team_user, db):
    with test_client as c:
        ...
        response = c.get('/db/item-blueprints/99999/edit')
...
def test_item_blueprints_delete_authorized(test_client, rules_team_user, item_blueprint_obj, db):
    with test_client as c:
        ...
        response = c.post(f'/db/item-blueprints/{item_blueprint_obj.id}/delete', follow_redirects=True)
        assert response.status_code == 200
...
def test_item_blueprints_delete_not_found(test_client, rules_team_user, db):
    with test_client as c:
        ...
        response = c.post('/db/item-blueprints/99999/delete', follow_redirects=True)
        assert response.status_code == 404 
