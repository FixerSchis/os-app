...
def test_items_edit_not_found(test_client, rules_team_user, db):
    with test_client.session_transaction() as session:
        session["_user_id"] = rules_team_user.id
        session["_fresh"] = True
    response = test_client.get("/db/items/99999/edit")
...
def test_items_delete_not_found(test_client, rules_team_user, db):
    with test_client.session_transaction() as session:
        session["_user_id"] = rules_team_user.id
        session["_fresh"] = True
    response = test_client.post("/db/items/99999/delete", follow_redirects=True)
    assert response.status_code == 404
...
def test_items_engineering_cost_item_maintain(test_client, item, db):
    response = test_client.post(
        "/db/items/engineering_cost", json={"item_id": item.id, "action": "maintain"}
    )
    assert response.status_code == 200
...
def test_items_engineering_cost_item_modify(test_client, item, db):
    response = test_client.post(
        "/db/items/engineering_cost", json={"item_id": item.id, "action": "modify"}
    )
    assert response.status_code == 200
    data = response.get_json()
    assert "cost" in data
...
def test_items_engineering_cost_blueprint_maintain(test_client, item_blueprint, db):
    response = test_client.post(
        "/db/items/engineering_cost",
        json={"blueprint_id": item_blueprint.id, "action": "maintain"},
    )
    assert response.status_code == 200
    data = response.get_json()
    assert "cost" in data
...
def test_items_engineering_cost_blueprint_modify(test_client, item_blueprint, db):
    response = test_client.post(
        "/db/items/engineering_cost",
        json={"blueprint_id": item_blueprint.id, "action": "modify"},
    )
    assert response.status_code == 200
    data = response.get_json()
    assert "cost" in data
...
def test_items_engineering_cost_invalid_action(test_client, item, db):
    response = test_client.post(
        "/db/items/engineering_cost", json={"item_id": item.id, "action": "invalid"}
    )
    assert response.status_code == 400
