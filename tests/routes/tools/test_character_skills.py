...
class TestCharacterSkills:
    ...
    def test_skills_list_forbidden(self, test_client, admin_character, character_with_faction, skill):
        other_user = User(
        ...
        db.session.commit()
        print(f"DEBUG: other_user.id={other_user.id}, other_user.player_id={other_user.player_id}, character.user_id={character_with_faction.user_id}, character.player_id={character_with_faction.player_id}")
    ...
    def test_get_skill_cost_valid(self, test_client, regular_user, character_with_faction, skill):
        login_user(test_client, regular_user)
        response = test_client.get(self.url(character_with_faction.id, f'/cost?skill_id={skill.id}'))
        assert response.status_code == 200
        data = response.get_json()
        assert data['can_purchase'] is True
        assert data['cost'] == skill.base_cost

    def test_get_skill_cost_already_has_skill(self, test_client, regular_user, character_with_faction, skill):
        cs = CharacterSkill(character_id=character_with_faction.id, skill_id=skill.id, times_purchased=1, purchased_by_user_id=regular_user.id)
        db.session.add(cs)
        db.session.commit()
        login_user(test_client, regular_user)
        response = test_client.get(self.url(character_with_faction.id, f'/cost?skill_id={skill.id}'))
    ...
    def test_purchase_skill_success(self, test_client, regular_user, character_with_faction, skill):
        login_user(test_client, regular_user)
        response = test_client.post(self.url(character_with_faction.id, '/purchase'), data={'skill_id': skill.id})
    ...
    def test_purchase_skill_already_has(self, test_client, regular_user, character_with_faction, skill):
        cs = CharacterSkill(character_id=character_with_faction.id, skill_id=skill.id, times_purchased=1, purchased_by_user_id=regular_user.id)
        db.session.add(cs)
        db.session.commit()
        login_user(test_client, regular_user)
    ...
    def test_purchase_skill_forbidden(self, test_client, admin_character, character_with_faction, skill):
        other_user = User(
            email=f"other_{uuid.uuid4().hex[:8]}@example.com",
            email_verified=True,
            first_name="Other",
    ...
    def test_refund_skill_success(self, test_client, user_admin, admin_character, skill):
        cs = CharacterSkill(character_id=admin_character.id, skill_id=skill.id, times_purchased=1, purchased_by_user_id=user_admin.id)
        db.session.add(cs)
        db.session.commit()
        login_user(test_client, user_admin)
        response = test_client.post(self.url(admin_character.id, '/refund'), data={'skill_id': skill.id})
        assert response.status_code == 302  # Should redirect on success
    ...
    def test_refund_skill_forbidden(self, test_client, regular_user, admin_character, skill):
        cs = CharacterSkill(character_id=admin_character.id, skill_id=skill.id, times_purchased=1, purchased_by_user_id=regular_user.id)
        db.session.add(cs)
        db.session.commit()
        login_user(test_client, regular_user)
        response = test_client.post(self.url(admin_character.id, '/refund'), data={'skill_id': skill.id})
    ...
    def test_refund_skill_prereq(self, test_client, user_admin, admin_character, skill, prerequisite_skill):
        login_user(test_client, user_admin)
        # Add both skills to the character
        cs1 = CharacterSkill(character_id=admin_character.id, skill_id=skill.id, times_purchased=1, purchased_by_user_id=user_admin.id)
        cs2 = CharacterSkill(character_id=admin_character.id, skill_id=prerequisite_skill.id, times_purchased=1, purchased_by_user_id=user_admin.id)
        db.session.add(cs1)
        db.session.add(cs2)
        db.session.commit()
        response = test_client.post(self.url(admin_character.id, '/refund'), data={'skill_id': prerequisite_skill.id})
        assert response.status_code == 302  # Should redirect on error
