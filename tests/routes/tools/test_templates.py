...
from models.enums import PrintTemplateType, EventType, ScienceType
...
class TestTemplatesRoutes:
    ...
    def make_template(self, admin_user, type_value, type_name, front_html='Hello'):
        return PrintTemplate(type=type_value, type_name=type_name, created_by_user_id=admin_user.id, front_html=front_html)

    def test_template_list(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CHARACTER_SHEET, "Character Sheet List")
    ...
    def test_template_new_post(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        resp = test_client.post('/templates/templates/new', data={
    ...
    def test_template_edit_get(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CONDITION_CARD, "Condition Card Edit")
        template.front_html = 'Edit Template'
        db.session.add(template)
        db.session.commit()
        resp = test_client.get(f'/templates/templates/{template.id}/edit')
        assert resp.status_code == 200
    ...
    def test_template_edit_post(self, test_client, admin_user, db):
        ...
        db.session.add(template)
        db.session.commit()
        resp = test_client.post(f'/templates/templates/{template.id}/edit', data={
    ...
    def test_render_template_preview(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.MEDICAMENT_CARD, "Medicament Card Preview", front_html='Hello')
        db.session.add(template)
        db.session.commit()
        resp = test_client.post(f'/templates/api/{template.id}/render', json={
            'front_html': 'Hello',
            'back_html': '',
            'css_styles': ''
        })
    ...
    def test_print_template_preview(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.EXOTIC_SUBSTANCE_LABEL, "Exotic Label Preview", front_html='Hello')
        db.session.add(template)
        db.session.commit()
        resp = test_client.post(f'/templates/api/{template.id}/print_preview', json={
            'front_html': 'Hello',
            'back_html': '',
            'css_styles': ''
        })
    ...
    def test_print_item_sheet_character(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CHARACTER_SHEET, "Character Sheet Print")
        db.session.add(template)
        db.session.commit()
        character = Character(name='Char', user_id=admin_user.id, species_id=1, faction_id=1, status='active')
    ...
    def test_print_item_sheet_item(self, test_client, admin_user, db):
        ...
        db.session.commit()
        blueprint = ItemBlueprint(name="Plasma RifleA", item_type_id=item_type.id, blueprint_id=1, base_cost=500, purchaseable=True)
    ...
    def test_print_item_sheet_condition(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CONDITION_CARD, "Condition Card Print")
        db.session.add(template)
        db.session.commit()
        condition = Condition(name='Cond')
        db.session.add(condition)
        db.session.commit()
        stage = ConditionStage(condition_id=condition.id, stage_number=1, rp_effect="", diagnosis="", cure="", duration=1)
    ...
    def test_print_item_sheet_medicament(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.MEDICAMENT_CARD, "Medicament Card Print")
        db.session.add(template)
        db.session.commit()
    ...
    def test_print_item_sheet_exotic(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.EXOTIC_SUBSTANCE_LABEL, "Exotic Card Print")
        db.session.add(template)
        db.session.commit()
    ...
    def test_print_event_items_characters(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CHARACTER_SHEET, "Character Sheet Event Print")
        db.session.add(template)
        db.session.commit()
        now = datetime.now()
        event = Event(
        ...
        db.session.add(character)
        db.session.commit()
        ticket = EventTicket(event_id=event.id, character_id=character.id, ticket_type='adult', price_paid=100.0, assigned_by_id=admin_user.id)
    ...
    def test_print_event_items_items(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.ITEM_CARD, "Item Card Event Print")
        db.session.add(template)
        db.session.commit()
        now = datetime.now()
        event = Event(
        ...
        db.session.add(item_type)
        db.session.commit()
        blueprint = ItemBlueprint(name="Plasma RifleB", item_type_id=item_type.id, blueprint_id=1, base_cost=500, purchaseable=True)
        db.session.add(blueprint)
        db.session.commit()
        item = Item(blueprint_id=blueprint.id, item_id=1)
        db.session.add(item)
        db.session.commit()
        character = Character(name='Char2', user_id=admin_user.id, species_id=1, faction_id=1, status='active')
        db.session.add(character)
        db.session.commit()
        ticket = EventTicket(event_id=event.id, character_id=character.id, ticket_type='adult', price_paid=100.0, assigned_by_id=admin_user.id)
        db.session.add(ticket)
    ...
    def test_template_layout_get(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CHARACTER_SHEET, "Character Sheet Layout")
        db.session.add(template)
        db.session.commit()
    ...
    def test_template_layout_post(self, test_client, admin_user, db):
        self.clear_templates(db)
        self.login_admin(test_client, admin_user)
        template = self.make_template(admin_user, PrintTemplateType.CONDITION_CARD, "Condition Card Layout")
        db.session.add(template)
        db.session.commit()
        resp = test_client.post(f'/templates/{template.id}/layout', json={
            'width_mm': 100,
            'height_mm': 50,
            'is_landscape': True
        })
