...
class TestResearchAssignees:
    ...
    def test_remove_assignee(
        ...
        character_research_with_stage,
    ):
        login_user(test_client, user_rules_team)
        response = test_client.post(
            f"/research/{research_project_with_stage.id}/assignees/"
            f"{character_research_with_stage.id}/remove",
            follow_redirects=True,
        )
        assert response.status_code == 200
    ...
    def test_edit_progress_get(
        ...
        response = test_client.get(
            f"/research/{research_project_with_stage.id}/assignees/"
            f"{character_research_with_stage.id}/progress"
        )
        assert response.status_code == 200
